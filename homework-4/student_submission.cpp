...
/*
 * TODO@Students: This is your kernel. Take a look at any dependencies and how
 * to parallelize it
 */
int mandelbrot_draw(const int x_resolution, const int y_resolution,
                    const int max_iter, const double x_stepsize,
                    const double y_stepsize, const int palette_shift,
                    unsigned char *img, const double power,
...
#pragma omp parallel for schedule(dynamic, 10) reduction(+ : pointsInSetCount)      \
    firstprivate(x, y, Z, C, k)
  for (int i = 0; i < y_resolution; i++) {
    for (int j = 0; j < x_resolution; j++) {
      ...
      // Apply the Mandelbrot calculation until the absolute value >= 2 (meaning
      // the calculation will diverge to infinity) or the maximum number of
      ...
      // If the maximum number of iterations was reached then this point is in
      // the Mandelbrot set and we color it black. Else, it is outside and we
      // color it with a color that corresponds to how many iterations there
      ...
      if (!no_output) {
        if (k == max_iter) {
          ...
          int index =
              (k + palette_shift) % (sizeof(colors) / sizeof(colors[0]));
...
int main(int argc, char **argv) {
  ...
  // The output on standard out needs to be the same as in the sequential
  // implementation.
  fprintf(stdout,
          ...
          "Stepsize x = %lf y = %lf\n",
          max_iter, x_resolution, y_resolution, VIEW_X0, VIEW_X1, VIEW_Y0,
          VIEW_Y1, x_stepsize, y_stepsize);
  ...
  // compute mandelbrot
  int numSamplesInSet =
      mandelbrot_draw(x_resolution, y_resolution, max_iter, x_stepsize,
                      y_stepsize, palette_shift, image, power, no_output);
  ...
  if (!no_output) {
    ...
    size_t bytes_written = fwrite(
        image, 1, x_resolution * y_resolution * sizeof(unsigned char[3]), file);
